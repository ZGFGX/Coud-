...
interface IKep1rHelper {
    function getQuoteLimit(uint gasUsed) external view returns (uint);
}

contract Kep1r is ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    /// @notice Kep1r Helper to set max prices for the ecosystem
    IKep1rHelper public KPRH;

    /// @notice EIP-20 token name for this token
    string public constant name = "kep1r";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "kep1r";
    ...
    /// @notice Worked a job
    event Kep1rWorked(address indexed credit, address indexed job, address indexed kep1r, uint block, uint amount);

    /// @notice Kep1r bonding
    event Kep1rBonding(address indexed kep1r, uint block, uint active, uint bond);

    /// @notice Kep1r bonded
    event Kep1rBonded(address indexed kep1r, uint block, uint activated, uint bond);

    /// @notice Kep1r unbonding
    event Kep1rUnbonding(address indexed kep1r, uint block, uint deactive, uint bond);

    /// @notice Kep1r unbound
    event Kep1rUnbound(address indexed kep1r, uint block, uint deactivated, uint bond);

    /// @notice Kep1r slashed
    event Kep1rSlashed(address indexed kep1r, address indexed slasher, uint block, uint slash);

    /// @notice Kep1r disputed
    event Kep1rDispute(address indexed kep1r, uint block);

    /// @notice Kep1r resolved
    event Kep1rResolved(address indexed kep1r, uint block);

    event AddCredit(address indexed credit, address indexed job, address indexed creditor, uint block, uint amount);

    /// @notice 1 day to bond to become a kep1r
    uint constant public BOND = 1 days;
    /// @notice 2 days to unbond to remove funds from being a kep1r
    uint constant public UNBOND = 2 days;
    /// @notice 1 days till liquidity can be bound
    uint constant public LIQUIDITYBOND = 4 days;

    /// @notice direct liquidity fee 0.77%
    uint constant public FEE = 77;
    uint constant public BASE = 19710;
    ...
    /// @notice tracks how much a kep1r has bonded
    ...
    /// @notice tracks when a kep1r was first registered
    mapping(address => uint) public firstSeen;

    /// @notice tracks if a kep1r has a pending dispute
    mapping(address => bool) public disputes;

    /// @notice tracks last job performed for a kep1r
    mapping(address => uint) public lastJob;
    /// @notice tracks the total job executions for a kep1r
    mapping(address => uint) public workCompleted;
    /// @notice list of all jobs registered for the kep1r system
    ...
    /// @notice list of all current Kep1rs
    mapping(address => bool) public Kep1rs;
    /// @notice blacklist of Kep1rs not allowed to participate
    mapping(address => bool) public blacklist;

    /// @notice traversable array of kep1rs to make external management easier
    address[] public kep1rList;
    ...
    function applyCreditToJob(address provider, address liquidity, address job) external {
        ...
        uint _liquidity = Kep1rLibrary.getReserve(liquidity, address(this));
    ...
    /**
     * @notice Implemented by jobs to show that a kep1r performed work
     * @param kep1r address of the kep1r that performed the work
     */
    function worked(address kep1r) external {
        workReceipt(kep1r, KPRH.getQuoteLimit(_gasUsed.sub(gasleft())));
    }
     ...
     */
    function workReceipt(address kep1r, uint amount) public {
        ...
        lastJob[kep1r] = now;
        _reward(kep1r, amount);
        workCompleted[kep1r] = workCompleted[kep1r].add(amount);
        emit Kep1rWorked(address(this), msg.sender, kep1r, block.number, amount);
    }

    /**
     * @notice Implemented by jobs to show that a kep1r performed work
     * @param credit the asset being awarded to the kep1r
     * @param kep1r address of the kep1r that performed the work
     * @param amount the reward that should be allocated
     */
    function receipt(address credit, address kep1r, uint amount) external {
        ...
        lastJob[kep1r] = now;
        IERC20(credit).safeTransfer(kep1r, amount);
        emit Kep1rWorked(credit, msg.sender, kep1r, block.number, amount);
    }

    /**
     * @notice Implemented by jobs to show that a kep1r performed work
     * @param kep1r address of the kep1r that performed the work
     * @param amount the amount of ETH sent to the kep1r
     */
    function receiptETH(address kep1r, uint amount) external {
        require(jobs[msg.sender], "receipt: !job");
        credits[msg.sender][ETH] = credits[msg.sender][ETH].sub(amount, "workReceipt: insuffient funds");
        lastJob[kep1r] = now;
        payable(kep1r).transfer(amount);
        emit Kep1rWorked(ETH, msg.sender, kep1r, block.number, amount);
    }
    ...
    /**
     * @notice Allows governance to change the Kep1rHelper for max spend
     * @param _kprh new helper address to set
     */
    function setKep1rHelper(IKep1rHelper _kprh) external {
        require(msg.sender == governance, "setKep1rHelper: !gov");
        KPRH = _kprh;
    }
    ...
    /**
     * @notice confirms if the current kep1r is registered, can be used for general (non critical) functions
     * @param kep1r the kep1r being investigated
     * @return true/false if the address is a kep1r
     */
    function iskep1r(address kep1r) external returns (bool) {
        _gasUsed = gasleft();
        return Kep1rs[kep1r];
    }

    /**
     * @notice confirms if the current kep1r is registered and has a minimum bond, should be used for protected functions
     * @param kep1r the kep1r being investigated
     * @param minBond the minimum requirement for the asset provided in bond
     * @param earned the total funds earned in the kep1rs lifetime
     * @param age the age of the kep1r in the system
     * @return true/false if the address is a kep1r and has more than the bond
     */
    function isMinkep1r(address kep1r, uint minBond, uint earned, uint age) external returns (bool) {
        _gasUsed = gasleft();
        return Kep1rs[kep1r]
                && bonds[kep1r][address(this)].add(votes[kep1r]) >= minBond
                && workCompleted[kep1r] >= earned
                && now.sub(firstSeen[kep1r]) >= age;
    }
     ...
     * @return true/false if the address is a kep1r and has more than the bond
     */
    function isBondedKep1r(address kep1r, address bond, uint minBond, uint earned, uint age) external returns (bool) {
        _gasUsed = gasleft();
        return Kep1rs[kep1r]
                && bonds[kep1r][bond] >= minBond
                && workCompleted[kep1r] >= earned
                && now.sub(firstSeen[kep1r]) >= age;
    }

    /**
     * @notice begin the bonding process for a new kep1r
    ...
    function bond(address bonding, uint amount) external nonReentrant {
        ...
        emit Kep1rBonding(msg.sender, block.number, bondings[msg.sender][bonding], amount);
    }

    /**
     * @notice get full list of kep1r in the system
     */
    function getKep1rs() external view returns (address[] memory) {
        return kep1rList;
    }

    /**
     * @notice allows a kep1r to activate/register themselves after bonding
    ...
    function _activate(address kep1r, address bonding) internal {
        if (firstSeen[kep1r] == 0) {
          firstSeen[kep1r] = now;
          kep1rList.push(kep1r);
          lastJob[kep1r] = now;
        }
        Kep1rs[kep1r] = true;
        _bond(bonding, kep1r, pendingbonds[kep1r][bonding]);
        pendingbonds[kep1r][bonding] = 0;
        emit Kep1rBonded(kep1r, block.number, block.timestamp, bonds[kep1r][bonding]);
    }

    /**
     * @notice begin the unbonding process to stop being a kep1r
    ...
    function unbond(address bonding, uint amount) external {
        ...
        emit Kep1rUnbonding(msg.sender, block.number, unbondings[msg.sender][bonding], amount);
    }
    ...
    function withdraw(address bonding) external nonReentrant {
        ...
        emit Kep1rUnbound(msg.sender, block.number, block.timestamp, partialUnbonding[msg.sender][bonding]);
        partialUnbonding[msg.sender][bonding] = 0;
    }

    /**
     * @notice allows governance to create a dispute for a given kep1r
     * @param kep1r the address in dispute
     */
    function dispute(address kep1r) external {
        require(msg.sender == governance, "dispute: !gov");
        disputes[kep1r] = true;
        emit Kep1rDispute(kep1r, block.number);
    }

    /**
     * @notice allows governance to slash a kep1r based on a dispute
     * @param bonded the asset being slashed
     * @param kep1r the address being slashed
     * @param amount the amount being slashed
     */
    function slash(address bonded, address kep1r, uint amount) public nonReentrant {
        ...
        _unbond(bonded, kep1r, amount);
        disputes[kep1r] = false;
        emit Kep1rSlashed(kep1r, msg.sender, block.number, amount);
    }

    /**
     * @notice blacklists a kep1r from participating in the network
     * @param kep1r the address being slashed
     */
    function revoke(address kep1r) external {
        require(msg.sender == governance, "slash: !gov");
        Kep1rs[kep1r] = false;
        blacklist[kep1r] = true;
        slash(address(this), kep1r, bonds[kep1r][address(this)]);
    }


    /**
     * @notice allows governance to resolve a dispute on a kep1r
     * @param kep1r the address cleared
     */
    function resolve(address kep1r) external {
        require(msg.sender == governance, "resolve: !gov");
        disputes[kep1r] = false;
        emit Kep1rResolved(kep1r, block.number);
    }
